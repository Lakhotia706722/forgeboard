...
import { Pencil, Trash2, Play, ChevronDown, ChevronUp } from 'lucide-react'
...
interface AgentRowProps {
  ...
  onUpdated: (agent: AgentOut) => void
}

export default function AgentRow({ agent, onEdit, onDelete, onUpdated }: AgentRowProps) {
  ...
  return (
    <>
      ...
      {/* Expandable runs panel */}
      {expanded && (
        <tr className="border-b border-gray-800 bg-gray-950">
          <td colSpan={7} className="px-6 py-4">
            <AgentRunsPanel agent={agent} onAgentUpdated={onUpdated} />
          </td>
        </tr>
      )}
    </>
